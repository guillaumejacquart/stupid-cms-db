...
function editContent(contentName, pageName, html, attrs, repeatIndex, callback){	
	if(attrs){
		var handledAttributes = attrs.filter(function(a){
	...
	fs.access(filepath, fs.F_OK, function(){
		fs.readFile(filepath, 'utf8', function(err, data){
			...
			if(elem.length === 1){
				...
				throw new Error('Selector matches ' + elem.length + ' elements');
			}
		});	
	})	
};
...
module.exports = function(dbArg){
	...
	return {
		...
		orderRepeatable: orderRepeatable,
		makeElemEditable: makeElemEditable,
		destroyElemEditable: destroyElemEditable
	};
}
